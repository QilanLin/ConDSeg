...
def extract_uncertainty_masks(y_pred):
    """
    处理预测掩码，提取前景不确定性和背景不确定性区域
    ...
    # 确保y_pred是2D数组，移除所有长度为1的维度
    y_pred = np.squeeze(y_pred)
    
    # 前景不确定性区域 (0.5 <= prob < 0.7)
    fg_uncertainty_mask = (y_pred >= 0.5) & (y_pred < 0.7)
    
    # 背景不确定性区域 (0.3 < prob < 0.5)
    bg_uncertainty_mask = (y_pred > 0.3) & (y_pred < 0.5)
    
    # 完整不确定性区域 (0.3 < prob < 0.7)
    full_uncertainty_mask = (y_pred > 0.3) & (y_pred < 0.7)
    ...
    # 完整不确定性掩码（用于保持与原来test_exp_unet目录中图像的兼容性）
    ...
    return fg_uncertainty_mask_img, bg_uncertainty_mask_img, full_uncertainty_mask_img, y_pred
...
def evaluate(model, save_path, test_x, test_y, size):
    ...
    for i, (x, y) in tqdm(enumerate(zip(test_x, test_y)), total=len(test_x)):
        ...
        with torch.no_grad():
            ...
            """ Evaluation metrics """
            score_1 = calculate_metrics(mask_tensor, p1)
            metrics_score_1 = list(map(add, metrics_score_1, score_1))
            
            # 提取前景和背景不确定性掩码
            fg_uncertainty_mask, bg_uncertainty_mask, full_uncertainty_mask, raw_pred = extract_uncertainty_masks(p1)
...
if __name__ == "__main__":
    ...
    model = UNetPlusPlus(n_channels=3, n_classes=1, bilinear=True)
